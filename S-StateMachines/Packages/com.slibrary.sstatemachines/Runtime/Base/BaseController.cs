...
namespace SLibrary.StateMachines
{
    ...
    public class BaseController<T> : MonoBehaviour, IStateMachine<T> where T : Enum
    {
        ...
        public virtual bool SetState(T state, bool forceChange = false)
        {
            ...
            if (debugLevel == 1)
                DebugMessage((response ? "Successfully " : "Failed to ") + (forceChange ? "<b>FORCE</b> " : "") + "SET STATE [" + lastState.ToString() + "], --> [" + state.ToString() + "]", response ? 1 : 2);
            return response;
        }
        ...
        public void DebugMessage(string message, int context = 0)
        {
            string printedMessage = "<b><color=lightblue>" + this.name + "</color></b> " + "<color=";
            switch (context)
            {
                case 0: // No Context
                    printedMessage += "gray";
